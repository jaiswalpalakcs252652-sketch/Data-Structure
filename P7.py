...
def build_huffman_tree(frequencies):
    ...
    while len(heap) > 1:
        ...
        print(Fore.YELLOW + "Merging Nodes : ", f"{left.char}({left.freq})", "and", f"{right.freq}({right.freq})")
        time.sleep(0.5)
    return heap[0]
...
def huffman_decoding(encoded_data, codebook):
    ...
    for bit in encoded_data:
        ...
        if current_code in reverse_codebook:
            ...
            print(Fore.CYAN + f"Decoding : {current_code} -> {reverse_codebook[current_code]}")
            current_code = ""
            time.sleep(0.5)
    return decoded_data
